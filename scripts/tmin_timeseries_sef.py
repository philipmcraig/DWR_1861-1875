...
def MetaData(origseries):
    ...
    """
    import pylab as pl
    print "##### processing Tmin metadata #####"
...
loc = 'Shields'
vcol = 5 # variable column (1 for mslp, 2 for Td, 4 for Tmax, 5 for Tmin, -1 for rain)
...
if loc.upper() in ['LONDON','PLYMOUTH','HOLYHEAD','WICK']:
    ...
    data1.to_csv(sefdir+loc+'A_tmin.csv',index=False,
                header=['year','month','day','hour','minute','tmin','meta'])
    ...
    # write data frame to .csv file
    data2.to_csv(sefdir+loc+'B_tmin.csv',index=False,
                header=['year','month','day','hour','minute','tmin','meta'])
if loc.upper() == 'GREENCASTLE':
    data = pl.array([yearseries,monthseries,dayseries,hours,mins,varseries,meta])
    data = pd.DataFrame(data.T)
    
    data.to_csv(sefdir+'Moville_tmin.csv',index=False,#cols=header,
                header=['year','month','day','hour','minute','tmin','meta'])
else:
    data = pl.array([yearseries,monthseries,dayseries,hours,mins,varseries,meta])
    data = pd.DataFrame(data.T)
    
    data.to_csv(sefdir+loc+'_tmin.csv',index=False,#cols=header,
                header=['year','month','day','hour','minute','tmin','meta'])
