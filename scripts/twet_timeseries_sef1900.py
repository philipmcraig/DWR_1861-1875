...
def Get2pmData(dwrdir,locstring,varstring,):
    ...
    if S[0].size == 1:
        print "##### 2pm data for " + locstring + " twet exists #####"
        data2pm = pl.array(df)
        twet2pm = (pl.array(df[locstring+' '+varstring])[:-1]-32)*(5./9.)
        ...
        meta2pm = pl.zeros_like(twet2pm,dtype='object')
        ...
        return twet2pm, meta2pm, years2pm, months2pm, days2pm#, hours2pm
...
print "##### running " + loc + " twet #####"

twet2pm, meta2pm, years2pm, months2pm, days2pm = Get2pmData(dwrdir,loc,'Wet')

twet_yr = pl.zeros([len(twet2pm)])
orig_yr = pl.zeros([len(twet2pm)])

Z = pl.where(pl.array(twet2pm)!=0)
twetseries = list(pl.array(twet2pm)[Z[0]])
...
X = pl.argwhere(pl.isnan(twetseries))

for i in range(len(X)):
    twetseries.pop(X[i,0]-i)
...
twetseries = pl.asarray(twetseries[:])
...
data = pl.array([yearseries,monthseries.astype(int),
                 dayseries.astype(int),hours,mins,
                    pl.around(twetseries,2),meta],dtype='object')
data = pd.DataFrame(data.T)

data.to_csv(sefdir+loc.replace(' ','')+'_twet2pm.csv',index=False,
            header=['year','month','day','hour','minute','twet','meta'])

print "##### completed " + loc + " twet #####"
