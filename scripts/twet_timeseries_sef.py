...
def MetaData(origseries):
    ...
    """
    import pylab as pl
    print "##### processing Twet metadata #####"
...
loc = 'Portrush'
vcol = 3 # variable column (1 for mslp, 2 for Td, 3 for Tw, -1 for rain)
vcol2pm = -1 # variable column for 2pm data (-3 for mslp, -2 for Td, -1 for Tw)
INC2PM = False
datebreak = '1872/06/27'#['1872/01/04','1872/03/22']
varlist = []; var2pmlist = []
    ...
    for i in range(len(dwrfiles)):
        ...
        if years[Y] == 1863:
            if logs[ind,vcol] == ' ' or logs[ind,vcol] == '  ':
                ...
                origyr[i] = pl.float32('nan')
                metalist.append('')
            elif logs[ind,vcol-1] in [999,'999',' 999.00',' 999',' NaN']:
                varyr[i] = pl.float32('nan')
                origyr[i] = pl.float32('nan')
                metalist.append('')
            elif isinstance(logs[ind][vcol],str) == True and logs[ind][vcol][0] == '?':
                varyr[i] = (int(logs[ind,vcol-1][1:]) - int(logs[ind,vcol][1:])-32)/1.8
                origyr[i] = int(logs[ind,vcol-1][1:]) - int(logs[ind,vcol][1:])
                metalist.append('?')
            elif isinstance(logs[ind][vcol-1],str) == True and logs[ind][vcol-1][0] == '?':
                varyr[i] = (int(logs[ind,vcol-1][1:]) - int(logs[ind,vcol])-32)/1.8
                origyr[i] = int(logs[ind,vcol-1][1:]) - int(logs[ind,vcol])
                metalist.append('?')
            else:
                varyr[i] = (int(logs[ind,vcol-1]) - int(logs[ind,vcol])-32)/1.8
                origyr[i] = int(logs[ind,vcol-1]) - int(logs[ind,vcol])
                metalist.append('')
        elif years[Y] == 1862 and i>=255:
            if logs[ind,vcol] == ' ' or logs[ind,1] == '  ':
                ...
                origyr[i] = int(logs[ind,vcol-1]) - int(logs[ind,vcol])
                metalist.append('')
        elif years[Y] >= 1872 or years[Y] == 1861 or years[Y] == 1862:
            if logs[ind,vcol] == ' ' or logs[ind,vcol] == '  ':
                varyr[i] = pl.float32('nan')
                origyr[i] = pl.float32('nan')
                metalist.append('')
...
if INC2PM == True and years.max()>=1872: # same needed for 2pm data if it exists
    ...
    if years[0] == 1872:
        varseries = CombineData(var2pmseries,varseries,0,float)
        origseries = CombineData(orig2pmseries,origseries,0,float)
        dateseries = CombineData(date2pmseries,dateseries,0,'S10')
        hourseries = CombineData(hour2pmseries,hourseries,0,float)
    else:
...
FRANCE = ['PARIS','LORIENT','BREST','ROCHEFORT','CAPGRIZNEZ','BIARRITZ','TOULON']

mins = pl.zeros_like(dayseries)
if loc.upper() in FRANCE:
    mins[:] = 51

if loc.upper() in ['PORTSMOUTH','PLYMOUTH','HOLYHEAD','WICK']:
    split = pl.where(pl.asarray(dateseries)==datebreak)[0][0]
    ...
    data1.to_csv(sefdir+loc+'A_twet.csv',index=False,
                header=['year','month','day','hour','minute','twet','meta'])
    ...
    # write data frame to .csv file
    data2.to_csv(sefdir+loc+'B_twet.csv',index=False,
                header=['year','month','day','hour','minute','twet','meta'])
    ...
    data3.to_csv(sefdir+loc+'C_twet.csv',index=False,
                header=['year','month','day','hour','minute','twet','meta'])
    ...
    data1 = pd.DataFrame(data1.T)
    # write data frame to .csv file
    data1.to_csv(sefdir+loc+'_twet.csv',index=False,
                header=['year','month','day','hour','minute','twet','meta'])
    ...
    # write data frame to .csv file
    data2.to_csv(sefdir+'Moville_twet.csv',index=False,
                header=['year','month','day','hour','minute','twet','meta'])
else:
    data = pl.array([yearseries,monthseries,dayseries,hours,mins,varseries,meta])
    data = pd.DataFrame(data.T)
    data.to_csv(sefdir+loc+'_twet.csv',index=False,#cols=header,
                header=['year','month','day','hour','minute','twet','meta'])