...
for i in range(len(years)):
    ...
    with open(ispddir+str(years[i])+'/'+loc+str(years[i])+'.txt','r') as f:
        data = f.readlines()
...
#    
    ...
    axx.plot(P[:,-1],label='ISPD 1900')
    #if i == 0:
    axx.plot(dwrdata,label='DWR 1900')
    #elif i == 1:
    ...
    pl.title(letters[i]+loc+' '+str(years[i])+', RMSE = '+str(round(rmse,2)))
