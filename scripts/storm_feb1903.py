...
cn = ax1.contour(X,Y,Mnew,norm=pl.Normalize(964,1060),
...
ax1.annotate('(c) scout ensemble mean & DWR observations',(-19.5,64),fontsize=12,
             bbox={'facecolor':'w'})
...
cs = ax2.contourf(X,Y,Snew,norm=pl.Normalize(0,9),#colors='grey',
...
errs = ErrorCalc(pres,Mnew,Snew,coords,lon,lat)
...
ax2.annotate('(d) scout ensemble spread & z-scores',(-19.5,64),fontsize=12,
             bbox={'facecolor':'w'})

GridLines(ax1,False,True,False,False)
...
#pl.savefig(jashome+'/mslp_obs_'+year+'_comp/mslp_'+savetime+'_obs_sprd_scout_poster.png')
