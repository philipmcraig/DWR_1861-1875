...
def Get2pmData(dwrdir,locstring,varstring,):
    ...
    if S[0].size == 1:
        ...
        return pres2pm, meta2pm, years2pm, months2pm, days2pm#, hours2pm
#    else:
...
print "##### running " + loc + " mslp #####"

pres2pm, meta2pm, years2pm, months2pm, days2pm = Get2pmData(dwrdir,loc,'Pressure')

pres_yr = pl.zeros([len(pres2pm)])
orig_yr = pl.zeros([len(pres2pm)])

Z = pl.where(pl.array(pres2pm)!=0)
presseries = list(pl.array(pres2pm)[Z[0]])
...
X = pl.argwhere(pl.isnan(presseries))

for i in range(len(X)):
    presseries.pop(X[i,0]-i)
...
presseries = pl.asarray(presseries[:])
...
data = pl.array([yearseries,monthseries.astype(int),
                 dayseries.astype(int),hours,mins,
                    pl.around(presseries,2),meta],dtype='object')
data = pd.DataFrame(data.T)

data.to_csv(sefdir+loc.replace(' ','')+'_mslp2pm.csv',index=False,
            header=['year','month','day','hour','minute','mslp','meta'])

print "##### completed " + loc + " mslp #####"
